  ...
  return (
    <div>
      {shoppingCart.length > 0 ? (
        <div
          className={`${cartStyles.font} table-responsive-md container mx-0`}
        >
          <Table hover>
            <thead>
              <tr>
                <th className="text-start">Producto</th>
                <th className="d-none d-md-table-cell text-center">Img</th>
                ...
                return (
                  <tr className="text-start">
                    ...
                    <td className="text-center align-middle">
                      <select
                        onChange={(ev) =>
                          updateQuantity(item.prod.id, ev.target.value)
                        }
                      ...
                      >
                        {cantidades.map((cant) => (
                          <option
                            value={cant}
                            key={cant}
                            selected={cant === Number(item.quantity)}
                          >
                            {cant}
                          </option>
                        ))}
